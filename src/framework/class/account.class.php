...
abstract class WeAccount
{
    ...
    public static function createByType($account_type = ACCOUNT_TYPE_OFFCIAL_NORMAL)
    {
        ...
        return self::includes(['type' => $account_type]);
    }
    ...
    public function errorCode($code, $errmsg = '未知错误')
    {
        ...
        if ($code == '40001' || $code == '42001') {
            cache_delete(cache_system_key('accesstoken', ['acid' => $this->account['acid']]));
...
class WeUtility
{
    ...
    public static function logging($level = 'info', $message = '')
    {
        ...
        if (is_string($message) && ! in_array($message, ['post', 'get'])) {
            $content .= "String:\n{$message}\n";
        }
        if (is_array($message)) {
            $content .= "Array:\n";
...
abstract class WeBase
{
    ...
    public function saveSettings($settings)
    {
        ...
        if (pdo_fetchcolumn("SELECT module FROM " . tablename('uni_account_modules') . " WHERE module = :module AND uniacid = :uniacid",
            [':module' => $this->modulename, ':uniacid' => $_W['uniacid']])) {
            $result = pdo_update('uni_account_modules', $row, $pars) !== false;
        } else {
            $result = pdo_insert('uni_account_modules', [
...
abstract class WeModuleProcessor extends WeBase
{
    ...
    protected function respCustom(array $message = [])
    {
        $response = [];
        $response['FromUserName'] = $this->message['to'];
        $response['ToUserName'] = $this->message['from'];
...
abstract class WeModuleSite extends WeBase
{
    ...
    protected function pay($params = [], $mine = [])
    {
        global $_W;
        load()->model('activity');
        load()->model('module');
        ...
        $log = pdo_get('core_paylog',
            ['uniacid' => $_W['uniacid'], 'module' => $params['module'], 'tid' => $params['tid']]);
        if (empty($log)) {
            $log = [
                'uniacid'    => $_W['uniacid'],
                'acid'       => $_W['acid'],
        ...
        $setting = uni_setting($_W['uniacid'], ['payment', 'creditbehaviors']);
        ...
        if ( ! empty($pay['credit']['switch'])) {
            ...
            $credit_pay_setting = mc_fetch($_W['member']['uid'], ['pay_password']);
    ...
    public function payResult($ret)
    {
        ...
        if ($ret['from'] == 'return') {
            if ($ret['type'] == 'credit2') {
                message('已经成功支付', url('mobile/channel', ['name' => 'index', 'weid' => $_W['weid']]), 'success');
            } else {
                message('已经成功支付', '../../' . url('mobile/channel', ['name' => 'index', 'weid' => $_W['weid']]),
                    'success');
            }
        }
    }
...
EOF;
    ...
    protected function click($params = [])
    {
        global $_W;
        $url = murl('utility/click', [
            'module' => $params['module'],
            'action' => $params['action'],
            'sign'   => $params['sign'],
            'tuid'   => $params['tuid'],
...
abstract class WeModuleWxapp extends WeBase
{
    ...
    protected function pay($order)
    {
        ...
        $paylog = pdo_get('core_paylog',
            ['uniacid' => $_W['uniacid'], 'module' => $this->module['name'], 'tid' => $order['tid']]);
        if (empty($paylog)) {
            $paylog = [
                'uniacid'    => $_W['uniacid'],
                'acid'       => $_W['acid'],
                ...
                'status'     => '0',
                'is_usecard' => '0',
                'tag'        => iserializer(['acid' => $_W['acid'], 'uid' => $_W['member']['uid']]),
            ];
    ...
    protected function wechatExtend($params)
    {
        ...
        $setting = uni_setting($wxapp_uniacid, ['payment']);
        $wechat_payment = [
    ...
    protected function creditExtend($params)
    {
        ...
        $credtis = mc_credit_fetch($_W['member']['uid']);
        $paylog = pdo_get('core_paylog',
            ['uniacid' => $_W['uniacid'], 'module' => $this->module['name'], 'tid' => $params['tid']]);
        if (empty($_GPC['notify'])) {
            ...
            $result = mc_credit_update($_W['member']['uid'], 'credit2', -$fee,
                [$_W['member']['uid'], '消费credit2:' . $fee]);
            if (is_error($result)) {
                return error(-1, $result['message']);
            }
            pdo_update('core_paylog', ['status' => '1'], ['plid' => $paylog['plid']]);
